...
const CoinPage = () => {
  ...
  const params = useParams();                            //App.js  route me jo bhi path diye honge ussey fetch kr lega param
        ...
        <div>
          <div className='flex justify-between'>
            {coin.market_data?.current_price ? (
              <p className='text-3xl font-bold'>${coin.market_data.current_price.usd.toString()}</p>) : null}
              <p>7 Day</p>
          </div>
          <div>
          ...
          <div className='flex justify-between py-4'>
            <div>
              ...
              {coin.market_data?.market_cap ? (<p>${coin.market_data.market_cap.usd.toString()}</p>):null}
            </div>
            <div>
              <p className='text-gray-500 text-sm'>Volume (24h)</p>
              {coin.market_data?.market_cap ? (<p>${coin.market_data.total_volume.usd.toString()}</p>):null}
            </div>
          </div>
          <div className='flex justify-between py-4'>
          <div>
            <p className='text-gray-500 text-sm'>24h High</p>
            {coin.market_data?.high_24h ? (<p>${coin.market_data.high_24h.usd.toString()}</p>):null}
          </div>
          <div>
            <p className='text-gray-500 text-sm'>24h Low</p>
            {coin.market_data?.low_24h ? (<p>${coin.market_data.low_24h.usd.toString()}</p>):null}
          </div>
          </div>
        </div>
        <div>
          ...
          <div>
            ...
            <div>
              <p className=''>Price Change (14d)</p>
